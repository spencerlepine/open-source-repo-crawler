...
const admin = {
  ...
  logEntries: async (redis) => {
    ...
    logger('[ADMIN LOG]', `queue:crawl: ${crawlQueue.join('\n')} (length: ${crawlQueue.length})`, 'warning');

    const analyzeQueue = await redis.lRange(CRAWLER_ANALYZE_KEY, 0, -1);
    logger('[ADMIN LOG]', `queue:analyze: ${analyzeQueue.join('\n')} (length: ${analyzeQueue.length})`, 'warning');
  },
}
