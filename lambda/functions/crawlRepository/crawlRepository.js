...
const getNextRepoFromCrawlQueue = async (redis) => {
  let nextQueueItem = await redis.lPop(CRAWLER_QUEUE_KEY) || 'public-apis/public-apis'
...
const isBrandNewLink = async (redis, repository) => {
  ...
  if (existingHashSet) {
    ...
    await redis.hSet(repository, 'hasBeenCrawled', 'false', 'hasBeenAnalyzed', 'false')
    return true
  }
}
